        ...
        portKeys: [
          {
            ...
            effect: ({
              value,
              context,
            }: EffectFuncProps<Array<string>, FormData>) => {
              const { node } = context;
              const defaultPorts: WorkflowPorts = [{ type: 'input' }];
              const newPorts: WorkflowPorts = value.map(
                (portID: string, i: number) => ({
                  type: 'output',
                  portID,
                  location: 'right',
                  locationConfig: {
          ...
          <Field<Array<string>> name="portKeys">
            {({ field: { value, onChange } }) => (
              ...
              >
                <div>
                  <button
                    onClick={() => onChange([...value, `if_${value.length}`])}
                  >
                    Add Port
                  </button>
                </div>
                <div style={{ marginTop: 8 }}>
                  <button
                    onClick={() =>
                      onChange(
                        value.filter((v, i, arr) => i !== arr.length - 1),
                      )
                    }
                  >
                    Delete Port

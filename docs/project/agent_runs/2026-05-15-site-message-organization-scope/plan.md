...
- Backend internal service calls preserve `x-organization-id`, so frontend organization switching is respected by downstream role checks.
...
- Frontend sends the current organization header through the existing request interceptor; no new UI control is needed for recipient selection because recipient scope is always the current organization.
